...
static int count = 0;
int information_greater_than_root(int rt, tree_type *root)
{
    if(root == NULL)
        return count;
    else if(root->right > rt || root->left > rt)
        count++;
    else
        return information_greater_than_root(rt,root->right);
}
...
int main()
{
    ...
    while(opt=='Y' || opt=='y');
    cout<<endl<<"Number of Nodes having Information Greater than Root : "<<information_greater_than_root(var,root)<<endl;
    return 0;
}
