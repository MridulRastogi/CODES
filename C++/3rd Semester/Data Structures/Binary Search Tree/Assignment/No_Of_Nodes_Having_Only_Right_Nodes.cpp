...
int count_nodes_having_only_right_child(tree_type *root)
{
    if(root==NULL)
        return 0;
    if(root->right==NULL)
        return count_nodes_having_only_right_child(root->left);
    else
        return (count_nodes_having_only_right_child(root->left) + count_nodes_having_only_right_child(root->right) + 1);
}

//IN THIS THE ROOT WILL NOT BE COUNTED AS THE NODE INSPITE OF HAVING A RIGHT CHILD.
...
int main()
{
    ...
    cout<<endl<<"Number of Nodes having only Right Child : "<<count_nodes_having_only_right_child(root)<<endl;
    return 0;
}
